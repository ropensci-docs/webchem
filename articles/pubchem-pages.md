# Retrieving data from PubChem pages
...
When we manually search for a compound on PubChem
(<https://pubchem.ncbi.nlm.nih.gov/>), we get a web page like this one:
<https://pubchem.ncbi.nlm.nih.gov/compound/1983>. This vignette explains
how most of this data can be retrieved programmatically using `webchem`.

## Getting started

First, we need to get the PubChem ID (CID) of the compound. This can be
done using the
[`get_cid()`](https://docs.ropensci.org/webchem/reference/get_cid.md)
function. Throughout this vignette, we will mostly work with
paracetamol, so let’s get its CID:
...
Now that we know the CID, we can retrieve any section of the PubChem web
page using
[`pc_sect()`](https://docs.ropensci.org/webchem/reference/pc_sect.md)
(short for PubChem Section). Here is an example:
...
Notice that the function returns a data frame rather than a string.
[`pc_sect()`](https://docs.ropensci.org/webchem/reference/pc_sect.md)
always returns a data frame but the variables it contains depends on the
section. The first five variable names shown above are common because
they follow a standard schema for sections whose data consists of a
single string, number, or similar value: the queried section, domain, ID
(e.g., CID or SID), name, and result. For simplicity, this vignette only
shows the first six variables of each tibble. Check the full output if
you want to see all available variables.

Returning a data frame offers several advantages over returning only the
result string. For example, we can query multiple compounds at the same
time and simply get a data frame with additional rows:
...
Another advantage of the data frame structure is that it also includes
any reference data stored in PubChem for each data element.

The `section` argument is not case sensitive but it is sensitive to
typing errors and requires the full name of the section as it is printed
on the content page. The PubChem Table of Contents Tree can also be
found at <https://pubchem.ncbi.nlm.nih.gov/classification/#hid=72>. As a
general rule,
[`pc_sect()`](https://docs.ropensci.org/webchem/reference/pc_sect.md)
works with the lowest level section before the data. For example, it
works with “IUPAC Name” but not with “Names and Identifiers”.
...
pc_sect(1983, "MeSH Entry Terms", form = "long") |> head(5) |> dplyr::select(1:6)
#> # A tibble: 5 × 6
#>   Section          Domain   ID    Name          Result                 SourceName                     
#>   <chr>            <chr>    <chr> <chr>         <chr>                  <chr>                          
#> 1 mesh entry terms compound 1983  Acetaminophen Acetaminophen          Medical Subject Headings (MeSH)
#> 2 mesh entry terms compound 1983  Acetaminophen Acetamidophenol        Medical Subject Headings (MeSH)
#> 3 mesh entry terms compound 1983  Acetaminophen Acetominophen          Medical Subject Headings (MeSH)
#> 4 mesh entry terms compound 1983  Acetaminophen Hydroxyacetanilide     Medical Subject Headings (MeSH)
#> 5 mesh entry terms compound 1983  Acetaminophen N-Acetyl-p-aminophenol Medical Subject Headings (MeSH)
```

Notice that we need to use `form = "long"` to get the desired result. So
far “MeSH Entry Terms” appear to be the only section where where `form`
needs to be set manually, but let us know if you find other sections.

## Chemical and Physical Properties

In the “Computed Properties” section, the lowest-level section names
that can be retrieved are the property names. For example:
...
The PubChem web page also displays a figure, which can be retrieved from
the URL in the `Thumbnail` variable. However, if we want to generate the
figure ourselves, the peaks are available in the
`Shifts [ppm]:Intensity` variable. Notice that this section returns
several data variables instead of a single `Result` variable. This is
because the retrieved data set is a data frame, so `form = "auto"`
detects this and converts the output to wide format.

## Drug and Medical Information

I included this section in the vignette because its output is a little
more complicated. When we look at this section of the PubChem web page
(<https://pubchem.ncbi.nlm.nih.gov/compound/1983#section=Drug-Indication>)
we see that it contains both a table and textual data. When we retrieve
the data,
[`pc_sect()`](https://docs.ropensci.org/webchem/reference/pc_sect.md)
returns the table in wide format while using the standard schema with
the `Result` column for the textual data. Downstream processing of this
data frame may require some extra attention.
...
We can retrieve clinical trial information from “EU Clinical Trials
Register”.
...
#>   <chr>                       <chr>    <chr> <chr>         <chr> <chr>         
#> 1 eu clinical trials register compound 1983  Acetaminophen <NA>  2007-000271-42
#> 2 eu clinical trials register compound 1983  Acetaminophen <NA>  2007-005619-24
#> 3 eu clinical trials register compound 1983  Acetaminophen <NA>  2004-001637-41
#> 4 eu clinical trials register compound 1983  Acetaminophen <NA>  2006-005956-34
#> 5 eu clinical trials register compound 1983  Acetaminophen <NA>  2006-006526-26
```
...
PubChem web pages contain a huge amount of information about chemicals.
Most of this data can be retrieved in `webchem` using
[`pc_sect()`](https://docs.ropensci.org/webchem/reference/pc_sect.md).
However, there may still be cases that are not handled properly. If you
find any examples where you are not satisfied with the output from
[`pc_sect()`](https://docs.ropensci.org/webchem/reference/pc_sect.md),
