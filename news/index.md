...
- [`pc_prop()`](https://docs.ropensci.org/webchem/reference/pc_prop.md)
  returned `NA` without much further explanation if any of the queries
  were not positive integers. The updated function attempts to coerce
  queries to positive integers, only progresses valid queries, and
  prints informative messages along the way if verbose messages are
  enabled.
...
- `is.inchikey(type = "chemspider")` broke because the URL was no longer
  working. We have updated the URL but the function now requires an API
  key like all other ChemSpider functions.
- [`pc_prop()`](https://docs.ropensci.org/webchem/reference/pc_prop.md)
  failed with `properties = NULL` because the list of accepted property
  names had changed. We have updated the list.
...
- The `nist_ri` function now returns a column with the CAS number to
  facilitate querying of multiple retention index tables.
- The `nist_ri` function can now take multiple arguments for `type`,
  `polarity` and `temp_prog`.
- CAS numbers are automatically reformatted using `as.cas` in
  `bcpc_query`, `cts_convert`, `get_etoxid`, `fn_percept`, `get_cid`,
  and `srs_query`.
...
- [`pan_query()`](https://docs.ropensci.org/webchem/reference/webchem-defunct.md)
  has been removed from the package because PAN no longer supports
  programmatic access.
...
- nist_ri() failed because the behaviour of a dependency had changed.
  This has been fixed.
- pc_sect() failed when query response was valid but the content was NA.
  This has been fixed.
...
- webchem can now access ChEMBL (<https://www.ebi.ac.uk/chembl/>) with
  `chembl_*()` functions. ChEMBL is a manually curated database of
  bioactive molecules with drug-like properties.

### BUG FIXES

- ChemSpider functions did not include time delays between queries. This
  has been fixed.
...
- get_cid() became more robust to smiles queries with special
  characters.
- `bcpc_query` now correctly parses the activity in cases that two
  activities are given (e.g. “herbicides” and “plant growth
  regulators”).
...
- Non-exported function ping_pubchem_pw() was incorrectly reporting that
  PUG VIEW was down. This has been fixed.
- is.cas() now catches whitespaces correctly.
- aw_query() was renamed and adapted to bcpc_query, as the alanwood site
  has moved.
...
- Download images of substances from Chemical Identifier Resolver (CIR)
  with
...
- [`find_db()`](https://docs.ropensci.org/webchem/reference/find_db.md)
  ...
  Useful for deciding which of several databases to focus on given a set
  of chemicals.
...
- The `"type"` argument in
  ...
  has been changed to `"from"` for consistency with other functions.
- [`fn_percept()`](https://docs.ropensci.org/webchem/reference/fn_percept.md)
  ...
  now have `"query"` and `"from"` arguments for consistency with other
  functions.
- Possible values for `"from"` have been made more consistent across
  functions.
- [`pc_synonyms()`](https://docs.ropensci.org/webchem/reference/pc_synonyms.md),
  ...
  have been changed to use the `match` argument instead of `choices` for
  consistency with other functions.
- [`get_etoxid()`](https://docs.ropensci.org/webchem/reference/get_etoxid.md)
  output changed slightly so that the matched chemical name string no
  longer includes the etoxid in parentheses.
- [`is.cas()`](https://docs.ropensci.org/webchem/reference/is.cas.md) is
  now vectorized.
...
- get_cid() now can search by registry IDs (e.g. CAS RN), and can handle
  more complex requests like searching for similar compounds.
...
- get_etoxid() now can search by CAS, EC, GSBL and RTECS numbers. Added
  `from =` argument.
- nist_ri() now can search by name, InChI, InChIKey, or CAS. The `cas`
  argument is deprecated. Use `query` instead with `from = "cas"`.

### MINOR IMPROVEMENTS

- All `get_*()` functions now output tibbles with a column for the query
  and a column for the retrieved ID.
- Changes to arguments in `get_*()` functions to make them more
  consistent.
...
- aw_query() returned a list for successful queries, NA for unsuccessful
  queries.
...
- Retrieve data from ChEBI (<https://www.ebi.ac.uk/chebi/>) webservice
  with get_chebiid() and chebi_comp_entity(). ChEBI comprises a rich
  data base on chemicals with bilogical interest.
...
- ChemSpider functions now look for an API token stored in .Renviron or
  .Rprofile by default so you can keep them hidden more easily.

### MINOR IMPROVEMENTS

- as.cas() added.
- Removed documentation files for non-exported functions that were only
  used internally.
...
- multiple functions failed because of a major change in the ChemSpider
  API.
...
- cs_extcompinfo() cannot be fixed as there is no equivalent in the new
  ChemSpider API yet.
...
- Added cts_to() and cts_from() to retrieve possible ids that can be
  queried.
- cts\_*(), pp_query(), cir_query(), get_cid(), get_etoxid(), etox\_*(),
  pan_query() get_wdid(), aw_query(), get_csid(), cs_prop(),
...
- etox\_\*() functions are more robust.
...
- alanwood() returns also subactvity (e.g. $`Fluazinam`$activity \[1\]
  “fungicides” and $`Fluazinam`$subactivity \[1\] “pyridine
  fungicides”).
...
- Changed etox\_\* functions to https.
...
- Query the ETOX ID with get_etoxid(); query basic information with
  etox_basic(); quality targets with etox_targets() and test results
  with etox_tests().
